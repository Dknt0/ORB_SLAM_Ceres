...
cv::Mat FrameDrawer::DrawFrame() {
  cv::Mat im;  // 图像
  vector<cv::KeyPoint>
      vIniKeys;  // 初始化参考帧中 KP  仅初始化中使用  Initialization: KeyPoints
                 // in reference frame
  vector<int> vMatches;  // 初始化匹配  仅初始化中使用  Initialization:
                         // correspondeces with reference keypoints
...
void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText) {
  ...
  int baseline = 0;
  cv::Size textSize = cv::getTextSize(s.str(), cv::FONT_HERSHEY_PLAIN, 1, 1,
                                      &baseline);  // 文字大小

  imText =
      cv::Mat(im.rows + textSize.height + 10, im.cols, im.type());  // 创建图像
  im.copyTo(imText.rowRange(0, im.rows).colRange(0, im.cols));  // 复制图像信息
  imText.rowRange(im.rows, imText.rows) = cv::Mat::zeros(
      textSize.height + 10, im.cols, im.type());  // 下方文字底色为黑色
